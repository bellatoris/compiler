...
program: ext_def_list {
		...
		sprintf(instruction,"Lglob.data %d",Global_Scope->offset); 
		Write_Else(instruction);
    }
   ;
...
ext_def
		...
		| func_decl {
		    ...
		    struct id* func_name = find_func($1)->name;
		    char instruction[100];
		    sprintf(instruction,"%s_final", func_name->name); 
		    Write_Label(instruction);
		    Write_Instruction("push_reg fp");
		    Write_Instruction("pop_reg sp");
		    Write_Instruction("pop_reg fp");
		    Write_Instruction("pop_reg pc");
		    sprintf(instruction,"%s_end", func_name->name); 
		    Write_Label(instruction);
		}
;
...
compound_stmt
		: '{'{
			...
			char instruction[100];
			sprintf(instruction, "shift_sp %d", $3);
			Write_Instruction(instruction);
			sprintf(instruction, "%s_start", func_name->name);
			Write_Label(instruction);
			in_func = 1;
		    }
		} stmt_list '}' {
...
stmt
		...
		| RETURN ';' {
		   ...
		   else
		   {
			...
			char instruction[100];
			sprintf(instruction, "jump %s_final", func_name->name);
			Write_Instruction(instruction);
		   }
		}
		| RETURN {
		    char instruction[100];
		    Write_Instruction("push_reg fp");
		    Write_Instruction("push_const -1");
		    Write_Instruction("add");
		    sprintf(instruction, "push_const -%d", finddecl(returnid)->size);
		    Write_Instruction(instruction);
		    Write_Instruction("add");		    
		} expr ';'{
			...
			char instruction[100];
			if($3->fetch)
			    Write_Instruction("fetch");
			Write_Instruction("assign");
			if($3->type->typeclass == Hash("struct"))
			{
			   int size = $3->size;
			   while(size > 1)
			   {
			       ...
			       Write_Instruction("push_const -1");
			       Write_Instruction("add");
			       sprintf(instruction, "push_const -%d", --size);
			       Write_Instruction(instruction);
			       Write_Instruction("add");
			       Write_Instruction("push_reg fp");
			       sprintf(instruction, "push_const %d", $3->str_offset + $3->size - size);
			       Write_Instruction(instruction);
			       Write_Instruction("add");
			       Write_Instruction("fetch");
			       Write_Instruction("assign");
		...
		| IF LABEL '(' expr ')' BRANCH stmt {
		    char instruction[100];
		    sprintf(instruction, "if_label_no_%d", $6 - 1);
		    Write_Label(instruction);
		}%prec ELSE 
		| IF LABEL '(' expr ')' BRANCH stmt ELSE  {
		    char instruction[100];
		    sprintf(instruction, "jump if_label_%d", $6);
		    Write_Instruction(instruction);
		    sprintf(instruction, "if_label_%d", $6 - 1);
		    Write_Label(instruction);
		} stmt {
		    char instruction[100];
		    sprintf(instruction, "if_label_%d", $6);
		    Write_Label(instruction);
		}%prec ELSE
		| WHILE {
		    char instruction[100];
		    sprintf(instruction, "loop_continue_%d", loop_continue_no++);
		    Write_Label(instruction);
			...
			Write_Instruction("fetch");
                    char instruction[100];
                    sprintf(instruction, "branch_false loop_out_%d", loop_out_no++);
                    Write_Instruction(instruction);
		    $<intVal>$ = loop_out_no;
		} stmt {
		    char instruction[100];
		    sprintf(instruction, "jump loop_continue_%d", $<intVal>2 - 1);
		    Write_Instruction(instruction);
		    sprintf(instruction, "loop_out_%d", $<intVal>6 - 1);
		    Write_Label(instruction);
		} 
		| FOR '(' expr_e ';' {
		    char instruction[100]; 
		...
		{
		    char instruction[100];
		    sprintf(instruction, "jump loop_continue_%d", loop_continue_no++);
		    Write_Instruction(instruction);
		    sprintf(instruction, "loop_continue_%d", loop_continue_no++);
		    Write_Label(instruction);
		    $<intVal>$ = loop_continue_no;
		    ...
		    char instruction[100];
		    sprintf(instruction, "jump loop_continue_%d", $<intVal>5 - 1);
		    Write_Instruction(instruction);
		} ')' {
		    char instruction[100];
		    sprintf(instruction, "loop_continue_%d", $<intVal>9 - 2);
		    Write_Label(instruction);
		} stmt {
		    char instruction[100];
		    sprintf(instruction, "jump loop_continue_%d", $<intVal>9 - 1);
		    Write_Instruction(instruction);
		    sprintf(instruction, "loop_out_%d", $<intVal>8 - 1);
		    Write_Label(instruction);
		}
		| BREAK ';'{
		    char instruction[100];
		    sprintf(instruction, "jump loop_out_%d", loop_out_no - 1);
		    Write_Instruction(instruction);
		}	
		| CONTINUE ';'{
		    char instruction[100];
		    sprintf(instruction, "jump loop_continue_%d", loop_continue_no - 1);
		    Write_Instruction(instruction);
		}
		| READ_INT '(' expr ')' {
		    Write_Instruction("read_int");
		}
		| READ_CHAR '(' expr ')' {
		    Write_Instruction("read_char");
		}
		| WRITE_INT '(' expr ')' {
		    if($3->fetch)
			Write_Instruction("fetch");
		    Write_Instruction("write_int");
		}
		| WRITE_CHAR '(' expr ')' {
		    if($3->fetch)
			Write_Instruction("fetch");
		    Write_Instruction("write_char");
		}
		| WRITE_STRING '(' expr ')' {
		    char instruction[100];
		    sprintf(instruction,"push_const str_%d",string_no++);
		    Write_Instruction(instruction);
		    Write_Instruction("write_string");
		}
;
LABEL		:{
		    char instruction[100];
		    sprintf(instruction, "if_label_%d", if_label_no++);      
		    Write_Label(instruction);
		    $$ = if_label_no;
		}
;
BRANCH		:{
			...
			Write_Instruction("fetch");
		    char instruction[100];
		    sprintf(instruction, "branch_false if_label_%d", if_label_no++);
		    Write_Instruction(instruction);
		    $$ = if_label_no++;
		}
;
		...
		: unary {
			...
			if(check_compatible($1->type, $4->type))
			{
				...
				while(size > 1)
				{
				    ...
				    {
					sprintf(instruction, "push_const Lglob+%d", $1->str_offset + $1->size - (--size));
					Write_Instruction(instruction);
				    }
				    else
				    {
					Write_Instruction("push_reg fp");
					sprintf(instruction, "push_const %d", $1->str_offset + $1->size - (--size));
					Write_Instruction(instruction);
					Write_Instruction("add");
				    }
				    if($4->scope == Global_Scope)
				    {
					sprintf(instruction, "push_const Lglob+%d", $4->str_offset + $1->size - (size));
					Write_Instruction(instruction);
				    }
				    else
				    {
					Write_Instruction("push_reg fp");		    
					sprintf(instruction, "push_const %d", $4->str_offset + $1->size - (size));
					Write_Instruction(instruction);
					Write_Instruction("add");
				    }
				    Write_Instruction("fetch");
...
binary
		: binary RELOP {
			...
			if(!strcmp($2, ">="))
			{
			    Write_Instruction("greater_equal");
			}
			else if(!strcmp($2, ">"))
			{
			    Write_Instruction("greater");
			}
			else if(!strcmp($2, "<"))
			{
			    Write_Instruction("less");
			}
			else
			{
			    Write_Instruction("less_equal");
			}
		    }
		    else
		    {
		...
		| binary EQUOP{
			...
			if(!strcmp($2, "=="))
			{
			    Write_Instruction("equal");
			}
			else
			{
			    Write_Instruction("not equal");
			}
		    }
		    else
		    {
		...
		| binary '+'{
		    ...
		    {
			$$ = makevardecl(plustype($1->type, $4->type));
			if($4->fetch)
			    Write_Instruction("fetch");
			$$->fetch = 0;
			Write_Instruction("add");
...
unary
		...
		| INTEGER_CONST {
		    ...
		    {
			char instruction[100];
			sprintf(instruction, "push_const %d", $1);
			Write_Instruction(instruction);
			$$->fetch = 0;
		    }
		}
		| CHAR_CONST {
		    ...
		    $$->fetch = 0;
		    char instruction[100];
		    sprintf(instruction,"push_const %d", $$->charvalue[0]);
		    Write_Instruction(instruction);
		}
		| STRING {
		    ...
		    $$->fetch = 0;
		    char instruction[100];
		    sprintf(instruction, "str_%d. string %s", string_no, $$->stringvalue);
		    Write_Else(instruction);
		}
		| ID {
		    if(finddecl($1))
			...
			{
			    char instruction[100];
			    sprintf(instruction, "push_const Lglob+%d", $$->offset);
			    Write_Instruction(instruction);
			    $$->fetch = 1;
			}
			else
			{
			    Write_Instruction("push_reg fp");
			    char instruction[100];
			    sprintf(instruction, "push_const %d", $$->offset);
			    Write_Instruction(instruction);
			    Write_Instruction("add");
			    $$->fetch = 1;
			}
		...
		| unary {
			...
			$$->fetch = 1;
			char instruction[100];
			sprintf(instruction, "push_const %d", $1->type->ptrto->size);
			if($4->fetch)
			{
			    Write_Instruction("fetch");
			}
			...
			$$->fetch = 1;
			char instruction[100];
			sprintf(instruction, "push_const %d", $1->type->elementvar->size);
			if($4->fetch)
			{
			    Write_Instruction("fetch");
			}
		...
		| unary '(' {
		   char instruction[100];
		   sprintf(instruction, "shift_sp %d", find_func($1)->decl->formals->decl->size);
		   Write_Instruction(instruction);
		   sprintf(instruction, "push_const label_%d", label_no);
		   Write_Instruction(instruction);
		   Write_Instruction("push_reg fp");
		} args ')'{
		    if(!$1 || !$4)
			...
			char instruction[100];
			Write_Instruction("push_reg sp");
			sprintf(instruction, "push_const -%d", $1->size);
			Write_Instruction(instruction);
			Write_Instruction("add");
			Write_Instruction("pop_reg fp");
			sprintf(instruction, "jump %s", func_name->name);
			Write_Instruction(instruction);
			sprintf(instruction, "label_%d", label_no++);
			Write_Label(instruction);
			if($1->returntype->typeclass == Hash("struct"))
			{
			}
		    }
		    else
...
args    /* actual parameters(function arguments) transferred to function */
		: args ',' expr {
			...
			{
			    char instruction[100];
			    if($3->scope == Global_Scope)
			    {
				sprintf(instruction, "push_const Lglob+%d", $3->offset + $3->size - (--size));
				Write_Instruction(instruction);
			    }
			    else
			    {
				Write_Instruction("push_reg fp");
				sprintf(instruction, "push_const %d", $3->offset + $3->size -(--size));
				Write_Instruction(instruction);
				Write_Instruction("add");
			    }
			    Write_Instruction("fetch");
		...
		| expr {
			...
			while(size > 1)
			{
			    ...
			    if($1->scope == Global_Scope)
			    {
				sprintf(instruction, "push_const Lglob+%d", $1->offset + $1->size - (--size));
				Write_Instruction(instruction);
			    }
			    else
			    {
				Write_Instruction("push_reg fp");
				sprintf(instruction, "push_const %d", $1->offset + $1->size-(--size));
				Write_Instruction(instruction);
				Write_Instruction("add");
			    }
			    Write_Instruction("fetch");
...
void init_type()
{
    ...
    Write_Instruction("shift_sp 1");
    Write_Instruction("push_const EXIT");
    Write_Instruction("push_reg fp");
    Write_Instruction("push_reg sp");
    Write_Instruction("pop_reg fp");
    Write_Instruction("jump main");
    Write_Label("EXIT");
...
struct ste *push_scope()	/* SStack.TOP에다가 새로히 ScopeNode를 만들고 연결 시킨다. */
{
    ...
    {
	SStack.TOP->str_offset = SStack.TOP->prev->str_offset;
    }
...
struct ste *insert(id *entry, struct decl *declptr)	/* declare와 똑같은 함수 이다. */
{
    ...
    {
	declptr->scope = SStack.TOP->top->prev->decl->scope;
    }
    declptr->offset = SStack.TOP->offset;
    declptr->str_offset = SStack.TOP->str_offset;
    SStack.TOP->offset += declptr->size;
    SStack.TOP->str_offset += declptr->size;
    if(SStack.TOP->top->name == returnid)
    {
	SStack.TOP->offset = 1;
	SStack.TOP->str_offset = 1;
    }
    return SStack.TOP->top;
}
